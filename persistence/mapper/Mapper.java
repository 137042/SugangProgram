...
public interface Mapper {
    ...
    @Select("SELECT plan FROM opened_lectures WHERE lecture_id=#{lecture_id} AND class_id=#{class_id}")
    List<OpenedLecturesDTO> selectOLPlanByLectureAndClassId(OpenedLecturesDTO dto);

    @Update("UPDATE opened_lectures SET professor_id=#{professor_id},building_id=#{building_id}," +
            "room_id=#{room_id},schedule=#{schedule},capacity=#{capacity},can_be_applied=#{can_be_applied} " +
